...
const Recipes = (props) => {
    if (props.drinkList.size === 0) {
        return <p>No selections. Use <FaPlusSquare /> icons to add to list.</p>;
    }

    const contents = transformSelectedCocktails(props.drinkList, props.cocktails, (cocktail, count) =>
        <div key={cocktail.id}>
            <CocktailHeading>{cocktail.name} <SmallText>(make {count})</SmallText>:</CocktailHeading>
